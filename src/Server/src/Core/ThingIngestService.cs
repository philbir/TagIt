...
public class ThingIngestService : IThingIngestService
{
        ...
        CancellationToken cancellationToken)
    {
        if (request.Action.Mode == JobActionMode.Import)
        {
            ...
            dataRef.Type = DataRefNames.Original;
    ...
    private async Task ResolveTypeAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (thing.TypeId is null && request.ContentType is { })
        {
